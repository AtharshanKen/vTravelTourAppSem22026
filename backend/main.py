from fastapi import FastAPI , HTTPException
from pydantic import BaseModel
from pydantic.v1 import BaseSettings
from dateutil import parser
import pandas as pd
import os
import datetime

#Adding env for open ai key, maps to .env for use for local work if no container env key
from openai import AsyncOpenAI
class Settings(BaseSettings):
    openai_api_key: str
    class Config:
        env_file = ".env"
        env_file_encoding = 'utf-8'

settings = Settings()

api = os.environ.get("OPENAI_API_KEY") # Switch between local/gcr
if api is None:
    api = settings.openai_api_key
client = AsyncOpenAI(api_key=api)

# Model Exc File
from knn_model import KNN_MD
from xgb_model import XGB_MD

folder1 = "./datasets"
dfs_comb = pd.DataFrame() # Load both data sets together
for df in [pd.read_csv(os.path.join(folder1,f)) for f in os.listdir(folder1) if f.endswith('daily.csv')]:
    dfs_comb = pd.concat([dfs_comb,df],axis='rows')
dfs_comb['Date'] = dfs_comb['Date'].apply(lambda x: parser.parse(x).date())

folder2 = "./datasets/flight_paths.csv"
flights = pd.read_csv(folder2) # Load Flight Data
flights['apt_time_dt_ds'] = flights['apt_time_dt_ds'].apply(lambda x: parser.parse(x).date())
flights['apt_time_dt_dp'] = flights['apt_time_dt_dp'].apply(lambda x: parser.parse(x).date())

app = FastAPI() # Initiate the Fast API End Points

def date_conv_to(df:pd.DataFrame,dates:list) -> list[dict]:
    df = df.copy()
    for cn in dates:
        df[cn] = pd.to_datetime(df[cn], errors="coerce").dt.strftime("%Y-%m-%d %H:%M:%S")
    return df.to_dict(orient="records")

def date_conv_from(df:pd.DataFrame,dates:list) -> pd.DataFrame:
    for cn in dates:
        df[cn] = pd.to_datetime(df[cn], errors="coerce").dt.date
    return df

@app.get("/")
def root():
    return {"message":"Hello"}

@app.get("/Health")
def backend_up():
    return {"status": "alive"}

@app.post("/dfs_flgh_data")
async def dfs_flgh_data(): # Return full flight data & historical data 
    return [date_conv_to(dfs_comb,['Date']),date_conv_to(flights,['apt_time_dt_ds','apt_time_dt_dp'])]

class FrCtReq(BaseModel):
    loc:str
    lat:float
    long:float
@app.post("/Forecasting") # Forecasting Function with XGB
async def forecasting(input:FrCtReq):
    fc = XGB_MD(input.loc,input.lat,input.long)
    return date_conv_to(fc,['Date'])

class RecReq(BaseModel):
    NewR:list
    loc:str
@app.post("/Recommendation") # Alt Dest with KNN model
async def recommendation(input:RecReq):
    input.NewR[8] = datetime.date.fromisoformat(input.NewR[8])
    kn = KNN_MD(input.NewR,dfs_comb,input.loc)
    kn['Date'] = pd.to_datetime(kn['Date'], errors="coerce").strftime("%Y-%m-%d %H:%M:%S")
    return kn.to_dict()


class open_AI(BaseModel):
    content:str 
@app.post("/OPENAI") # Open AI Requester
async def openai_api(input:open_AI):
    try:
        resp =  await client.responses.create(
                model="gpt-4o-mini",
                input = input.content,
                temperature=0.2,
                tools=[{"type": "web_search"}]
        )
        return {"resp":resp.output_text}
    except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))